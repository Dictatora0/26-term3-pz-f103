...
//	LCD_WriteCmd(0x21);		//设置颜色取反	
	...
	LCD_WriteCmd(0xBC);	
	LCD_WriteData(0X12);//亮度值	
	LCD_WriteData(0x17);//对比度值	
	LCD_WriteData(0x2F);//饱和度值	
	LCD_WriteData(0XF1);	
	delay_ms(10);	 
#endif
...
{  
	...
	height=ey-sy+1;			//高度
	count=(u32)width*height;

	LCD_Set_Window(sx, sy, ex, ey);
	for(idx=0; idx<count; idx++)
	{
		LCD_WriteData_Color(color[idx]);
	}
}//画点
//x,y:坐标
//FRONT_COLOR:此点的颜色
