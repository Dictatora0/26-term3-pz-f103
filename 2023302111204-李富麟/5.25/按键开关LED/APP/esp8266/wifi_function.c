...
{
    char cCmd [160];
    sprintf ( cCmd, "AT+MQTTCONNCFG=0,120,0,\"\",\"\",0,0" );
...
bool ESP8266_STA_TCP_Client_MQTT ( void )
{
    ...
    PC_Usart("[ESP] try baud=9600 (official default)\r\n");
    WiFi_SetUsart3Baud(9600);
    ESP8266_AT_Test();
    ok = ESP8266_Cmd("AT", "OK", NULL, 300);

    if (!ok) {
        PC_Usart("[ESP] no AT at 9600, try 115200...\r\n");
        PC_Usart("[ESP] try baud=115200\r\n");
        WiFi_SetUsart3Baud(115200);
        ESP8266_AT_Test();
        ok = ESP8266_Cmd("AT", "OK", NULL, 400);
    }

    if (!ok) {
        PC_Usart("[ESP] no response at 9600/115200.\r\n");
        PC_Usart("[ESP] If your module AT is 74880, use USB-UART first then set AT+UART_CUR=115200,8,1,0,0\r\n");
        return false;
    }
...
void ESP8266_MQTT_Task(void)
{
    ...
    if (strstr(rx, "+MQTTSUBRECV:") != NULL) {
        ...
        (void)ESP8266_STA_TCP_Client_MQTT();
    }
}
